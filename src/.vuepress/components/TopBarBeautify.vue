...
const CheckScrollTopClass = () => {
    ...
    if (blogHeroElm) {
        themeElm.classList.add('blog-hero');
    } else {
        themeElm.classList.remove('blog-hero');
    }

    const scrollTop = document.documentElement.scrollTop;
    if (scrollTop < 60) {
        themeElm.classList.add('top');
    } else {
        themeElm.classList.remove('top');
    }

    if (blogHeroElm) {
        if (scrollTop < blogHeroElm.clientHeight - 30) {
            themeElm.classList.add('blog-hero-inner');
        } else {
            themeElm.classList.remove('blog-hero-inner');
        }
    }
...
[data-theme='light'] {
    ...
    .theme-container.blog-hero-inner {
